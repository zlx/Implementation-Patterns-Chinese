...
假如你需要实例特定的行为，但是仅限于一两个方法，并且你不介意所有的多样性都集中在一个类里面。这种情况下，将你要调用的方法名存储在一个变量里面，然后通过反射来调用。

最初的时候，JUnit 里面的每个测试都是存储在自己的类里面的（Figure 5.7）。每个子类都只有一个方法。这里作为一个单独的类显得太重了。

[Figure 5.7]()

通过实现一个通用的 runTest(), 不同名字的 ListTests 运行不一样的测试方法。并且假定测试的名字就是但测试运行时恢复和运行的方法名。这里是一个最简单的实现可插拔选择器的例子来运行一个测试。
...
最简单的类继承树是一个单独的类（Figure 5.8）。就像所有代码压缩技术一样，如果你理解了这种诡计，修改后的代码看起来也很容易。

当可插拔选择器刚开始普及起来的时候，人们经常过度使用它。你可能看到过一些代码，删除了这些看起来从来没被被调用过代码，导致了系统挂了，因为它们被某个地方的可插拔选择器调用了。使用可插拔选择器的代价是值得考虑的，但是用它来解决麻烦的问题是值得的。
...
Java 提供了另外一种选择来实现实例特定的行为---匿名内部类。它可以创建一个仅在一个地方使用的类，它可以因为一些本地需要来重写一些方法。因为它只能在一个地方使用，这个类不能直接通过名字引用。

有效地使用匿名内部类依赖于是否有一个简单的外部 API---比如使用 run() 来实现 Runable() ---或者是有一个父类实现了大部分的内容，匿名内部类只需要实现一小部分就可以了。匿名内部类的代码中断了它所在代码的展现，所以它应该尽可能地简短。

匿名内部类的一个限制是你必须在运行前知道代码逻辑（不像代理可以之后设置），并且一旦一个实例创建出来就不能改变了。匿名内部类单独测试的时候是表现不一样的，所以它不能包含特别复杂的逻辑。因为它们是不具名的，所以你不能给匿名内部类取一个精心挑选的名字来表达你的意图。
