...
就像程序有主流程，它们也有一个或多个异常流程。有一些计算路径是较少被执行，较少会修改，相比主流程而言不重要一些的，它们的沟通要求就会低一些。清晰地表达主流程，然后保证这些异常流程尽可能不要复杂化主流程。 监护子句和异常是表达异常流程的两种方式：
...
因为有一个主流程，那么就有些情况会偏离这个主流程。 监护子句就是通过临时结果来表示简单的临时情况。比较下面两种情况：
...
如果我读到第一个版本，我会尝试去寻找 else 子句。我会在心里把条件放在栈中。这些都会让我在读 then 语句时分心。第二个版本的前两句都会让我知道一个事实：接受者还没有初始化。

条件语句和控制语句一样重要。监护子句对于表达不同的情况非常有效，特别在在某些时候，一个子句比另外一个子句更重要时。在上面初始化的例子，初始化对象的子句要更重要一些。另外的子句则表示，即使多次调用初始化，初始化都只会执行一次。

回到编程的旧时代，经常有种说法：每个方法都只能有一个入口，一个出口。这是为了避免在一个方法里面相互跳转导致混淆。这个在 FORTRAN 语言或者大量全局变量的语言中尤为明显，那样找到那个子句在执行是非常困难的。在 Java 里面，通过小方法和临时变量，不需要这么谨慎。然而，这个民俗并没有考虑其它人的感受，避免使用监护子句。
...
有些时候，监护子句会使用 continue 语句。他就是说：不要管他，处理下一个吧。
...
再次强调，关键在于指出正常流程和异常处理的不同。

### 异常

异常对于从多层调用里面跳出很有用。如果你发现在调用堆栈很深的地方发生了一个异常--磁盘满了或者网络断了--你只能做的就是退出调用堆栈。在出现错误的地方抛出，然后在可以处理的地方捕获，要比到处混杂错误检查代码来检查所有可能的错误要好。

异常的代价。他是一个设计的漏洞。事实上，调用方法返回一个异常可能影响所有调用了这个方法的思想和设计，知道这个方法的异常被捕获。这导致了很难去追踪这个控制流程，因为不同方法，对象和包里面的相关语句可能不一样。代码可以根据条件或者消息来写，但是通过异常来实现的代码会非常难读，因为你永远不知道什么时候会执行代码。总而言之，尽可能使用语句，消息，迭代和条件来表达控制流程。尽在没办法用正常流程表示时使用异常。

### 可检查异常

异常的一个危险是，当你抛出一个异常却没人处理它时该怎么办。那时候，程序就会退出。但是你希望能够在程序异常退出时有效控制，打印一些必要的信息来告诉用户发生了异常的时候发生了什么。

当抛出异常的代码和捕获异常的代码是不同的人写的时候，这件事情会变得更加困难。任何缺乏沟通的表现都会导致突然而意外地程序退出。

为了避免这种情况， Java 提供了可检查异常。它们是程序员明确声明，并由编译期检查的。包含了可检查异常的代码必须捕获这个异常或者继续向上抛出。

可检查异常通常和他的代价一起被提及。首先第一个代价是声明自己的代价。它们会很容易就增加 50% 的方法长度，导致了抛出异常和处理异常的人都增加了理解的成本。可检查异常也导致了修改代码变得困难。包含了可检查异常的代码的重构要困难许多，尽管使用了现代 IDE。

### 异常传递

异常出现在不同的抽象层次。捕获和报告一个低层次的异常可能会让某些人感到困惑。当一个 Web Server 抛出一个 NullPointerException ，我不知道我应该怎么处理这个消息。我更希望他告诉我：“程序不能处理你提供的情景。”我不介意这个页面是不是有我之前提交的信息，用来分析发生的情况，但是展现原始的细节是没什么用的。

低层次的异常通常会包含一些利于诊断缺陷的信息。用一个高层次的异常来包装低层次异常来保存这些信息，比如在 log 里面打印出足够用于判断实际情况的信息。

### 结论：

程序的方法间的控制流程是对象内建的。下一章节，我们将介绍如何使用方法来更好地表达计算过程。
